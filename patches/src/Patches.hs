...
patchContent = [
    ...
    {- "LitG"                                                 : some literal                                          -} "\"literal\"",
    {- "ListG EmptyG Nothing"                                 : list of something                                     -} "[...]",
    {- "TupleG ([VarG])"                                      : some n-tuple of variables                             -} "(x, y, ..., z)",
    ...
    {- "BopG (AppG [EmptyG]) (AppG [EmptyG])"                 : binary op on two function calls                       -} "(a b) + (x y z)",
    {- "TupleG ([LitG,ListG EmptyG Nothing])"                 : hetrogenous tuple of literals and lists of somethings -} "(x, \"literal\", ..., [...], [...])",
    {- "BopG VarG LitG"                                       : binary op on a var and a literal                      -} "x * \"literal\"",
    {- "BopG (BopG EmptyG EmptyG) LitG"                       : binary op on some binary op and literal               -} "(_ * _) + \"literal\"",
    {- "AppG ([AppG [EmptyG],LitG])"                          : apply some function to function calls and literals    -} "f (x y z) \"literal\" (g x)",
    ...
    {- "ListG LitG Nothing"                                   : list of literals                                      -} "[\"literal\", \"literal\", ..., \"literal\"]",
    ...
    {- "ConAppG (Just (TupleG ([VarG]))) Nothing"             : apply ctor to a tuple of variables                    -} "Ctor (x, y)",
    ...
    {- "AppG ([ConAppG Nothing (Just (TApp \"unit\" []))])    : fun app on ctor apps with no arguments                -} "f True False False"
  ]
